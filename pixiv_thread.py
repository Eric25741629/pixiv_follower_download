...
class get_following(QThread):
    ...
    def _flush_following_snapshot(self):
        try:
            ...
            # 同步寫入 txt + json，方便下次直接接續
            with open((self.path+"/following.txt"), "w+", encoding='utf-8') as f:
...
class get_pixiv_author_imgID_Thread(QThread):
    ...
    def run(self):
        ...
        try:
            # always commit collected pictures ids (even if stopped) to preserve work done so far
            pics_file = os.path.join(self.path, 'pictures_id.txt')
            # combine end (results) with any collected pids from workers
            try:
                with self._collected_pids_lock:
                    collected = list(self._collected_pids)
            except Exception:
                collected = list(getattr(self, '_collected_pids', []))
            # preserve order, avoid duplicates, and skip existing in exist_pid
            final_list = []
            seen_local = set()
...
class get_img_url_thread(QThread):
    ...
    def __init__(self,Author_list,Agent,cookies,exist_pid,ban_tag,must_tag,like_num,no_to_check, single_thread_mode=False, pid_wait_min=10, pid_wait_max=60):
    ...
    def _sleep_ultra_slow(self, pid):
        if not self.single_mode_flag:
            return
        delay = pyrandom.randint(self.pid_wait_min, self.pid_wait_max)
        try:
            self._output.emit("<p><font color='green'>[URL階段] 開始等待 {} 秒 (PID {})</font></p>".format(delay, pid))
        except Exception:
            pass
    ...
    def check_exist(self):
        try:
            with open((self.path+r"/pictures_id.txt")) as file:     #讀取寫入的文檔 if line.rstrip() not in self.no_to_check
                pictures_id = [line.rstrip() for line in file if line.rstrip() not in self.no_to_check]
                return pictures_id
        except:
        ...
        try:
            ...
            if self._isPause==2:
                ...
                with open(file_path + "/" + file_name, "a+") as f:
                    # 使用文件對象的 writelines 方法就可以一次寫入多行文本
                    f.writelines([str(text) + "\n" for text in tag_err])
...
class download_thread(QThread):
    ...
    def _sleep_between_downloads(self, pid):
        ...
        if not self.single_mode_flag:
            return
        delay = pyrandom.randint(self.download_wait_min, self.download_wait_max)
        try:
            self._output.emit("<p><font color='green'>[下載階段][PID間] 等待 {} 秒 (PID {})</font></p>".format(delay, pid))
        except Exception:
            pass
        for remaining in range(int(delay), 0, -1):
    ...
    def _sleep_within_pid(self, pid):
        ...
        if not self.single_mode_flag:
            return
        delay = pyrandom.randint(self.intra_pid_wait_min, self.intra_pid_wait_max)
        try:
            self._output.emit("<p><font color='gray'>[下載階段][同PID] 等待 {} 秒 (PID {})</font></p>".format(delay, pid))
        except Exception:
            pass
        for remaining in range(int(delay), 0, -1):
            if self._isPause == 2:
