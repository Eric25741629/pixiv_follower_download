...
Step 4 stamps each file with an advancing per-file timetag (`self.download_time`, +1 s per file under `timelock`), embedded in the filename and (when `download.set_file_mtime`, default true) applied to the file's atime/mtime via `_apply_download_mtime`. The counter is persisted through `WorkerEvent("timechanged", "YYYY-MM-DD HH:MM:SS")`: `download_thread._emit_timechanged()` fires at **every PID completion** (single + pool modes) and at finalize, and combined mode emits it per PID in its run loop; `flet_app.handle_timechanged` writes it back to `download.download_time`. Without that handler the counter was never persisted and every run restarted from the same stored value — mass duplicate filename prefixes. `_apply_live_settings_if_changed` only re-applies the stored value on a genuine user edit (tracked via `_download_time_setting_raw`) so the GUI write-back can't rewind the live counter. The settings view exposes 「下載時間戳起點」 and the mtime switch.
