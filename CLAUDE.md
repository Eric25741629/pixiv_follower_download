...
Step 4 stamps files with an advancing timetag (`self.download_time`, under `timelock`), embedded in the filename prefix and (when `download.set_file_mtime`, default true) applied to the file's atime/mtime via `_apply_download_mtime`. **The unit is the PID, not the file: every page of one artwork shares ONE timestamp** (so an artwork's pages get the same prefix / mtime and group as a unit). `_download_pid_group` — the single download choke point for every Step 4 path (single / pool / scheduler) **and** combined mode — calls `_begin_pid_timetag_block()` at the start of a PID's pages (reserves one stamp `base`, advances the global counter by exactly 1 s under `timelock`) and `_end_pid_timetag_block()` in `finally`; `_reserve_one_timetag()` returns that `base` for every page while a block is active (no per-page advance), else falls back to the legacy +1 s-per-call path. Distinct PIDs therefore get distinct stamps and concurrent PIDs reserve disjoint stamps; **uniqueness of each file is carried by `PID{pid}{page_suffix}`, not the timetag, so a shared stamp is collision-safe**. The counter is persisted through `WorkerEvent("timechanged", "YYYY-MM-DD HH:MM:SS")`: `download_thread._emit_timechanged()` fires at **every PID completion** (single + pool modes) and at finalize, and combined mode emits it per PID in its run loop; `flet_app.handle_timechanged` writes it back to `download.download_time`. Without that handler the counter was never persisted and every run restarted from the same stored value — mass duplicate filename prefixes across different PIDs. `_apply_live_settings_if_changed` only re-applies the stored value on a genuine user edit (tracked via `_download_time_setting_raw`) so the GUI write-back can't rewind the live counter. The settings view exposes 「下載時間戳起點」 and the mtime switch. (`tools/fix_duplicate_timetags.py` repairs the old cross-PID-collision corruption **PID-aware**: it de-dupes by PID, never splitting one PID's pages apart.) Tests: `tests/test_per_pid_shared_timetag.py`, `tests/test_fix_duplicate_timetags.py`.
