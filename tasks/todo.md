...
- [ ] **`metadata_db.py`(1448)**:先抽 `metadata_db_cache.py`(closed-set 快取 `_compute_closed_artwork_set`/`closed_artwork_set`/`_db_file_signature` + module 級 `_CLOSED_SET_CACHE`,自帶測試 `test_closed_set_cache.py`,風險低);再考慮 artwork/pages reader-writer 拆分(medium)。
- [ ] **`thread_download.py`(3080)**:`step4_jxl_conversion.py`(JXL 背景轉檔,~280)、`step4_filename.py`(檔名/標籤正規化,~130,需把 self.* 讀取轉成參數)、再到高風險 `step4_url_filter.py`/`step4_ugoira.py`/`step4_download_execute.py`(碰 scheduler/stop/url_meta/exist_pid 共享狀態)。
